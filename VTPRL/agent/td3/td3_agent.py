...
class TD3Agent:
    ...
    def _build_critic(self, s_dim, a_dim):
        ...
        class TwinCritic(nn.Module):
            def __init__(self):
                super().__init__()
                # Q1
                self.l1 = nn.Linear(s_dim + a_dim, 512)
                self.l2 = nn.Linear(512, 512)
                self.l3 = nn.Linear(512, 1)
                # Q2
                self.l4 = nn.Linear(s_dim + a_dim, 512)
    ...
    def select_action(self, state, noise=0.0):
        ...
        # 2. Handle Single Observation vs Batch
        # Case A: Single Obs (368,) -> unsqueeze to (1, 368)
        # Case B: Batch Obs (9, 368) -> keep as (9, 368)
        if state.dim() == 1:
        ...
        if noise != 0:
            # Noise shape matches action shape automatically
