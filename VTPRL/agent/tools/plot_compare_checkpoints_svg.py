...
CSV_PATH = ROOT / "logs" / "compare_checkpoints.csv"
OUT_PATH = ROOT / "logs" / "compare_checkpoints_bars_latex.svg"
PDF_PATH = ROOT / "logs" / "compare_checkpoints_bars_latex.pdf"
...
def normalize_label(name: str) -> str:
    return name.replace("td3_bc_offline_", "").replace("step_", "")
...
def build_panel(
    ...
    plot_w = right - left
    plot_h = bottom - top
    bar_w = plot_w / max(len(values) * 1.8, 1)
    gap = bar_w * 0.8
    start_x = left + gap / 2
    ...
    for gv in grid_values:
        ...
        parts.append(f'<text x="{left - 10}" y="{y + 4:.1f}" text-anchor="end" class="tick">{fmt(gv, digits if ymax <= 2 else 0)}</text>')
    ...
    for idx, value in enumerate(values):
        ...
        parts.append(
            f'<rect x="{x:.1f}" y="{y:.1f}" width="{bar_w:.1f}" height="{h:.1f}" fill="{color}" stroke="#000000" stroke-width="1.1"/>'
        )
        parts.append(
            f'<text x="{x + bar_w / 2:.1f}" y="{y - 8:.1f}" text-anchor="middle" class="value">{fmt(value, digits)}</text>'
        )
        parts.append(
            f'<text x="{x + bar_w / 2:.1f}" y="{bottom + 18:.1f}" text-anchor="middle" class="tick" transform="rotate(-25 {x + bar_w / 2:.1f},{bottom + 18:.1f})">{labels[idx]}</text>'
        )

    parts.append(f'<text x="{x0 + width / 2:.1f}" y="{y0 + height - 10}" text-anchor="middle" class="label">Checkpoint</text>')
    return "\n".join(parts)
...
def build_panel_pdf(
    ...
    colors = [
        ...
        (0x4A / 255.0, 0x55 / 255.0, 0x68 / 255.0),
        (0x71 / 255.0, 0x80 / 255.0, 0x96 / 255.0),
        (0xA0 / 255.0, 0xAE / 255.0, 0xC0 / 255.0),
    ]
    ...
    for gv in grid_values:
        ...
        tick_text = fmt(gv, digits if ymax <= 2 else 0)
        parts.append(pdf_text(left - 38, fy(y + 4), 10, tick_text))
    ...
    for idx, value in enumerate(values):
        ...
        parts.append(pdf_rotated_text(x + bar_w / 2 - 8, fy(bottom + 16), 10, -25, labels[idx]))

    parts.append(pdf_text_center(x0 + width / 2, fy(y0_top + height - 12), 13, "Checkpoint"))
    return parts


def write_pdf(rows: List[Dict[str, str]], labels: List[str], success: List[float], reward: List[float], steps: List[float]) -> None:
    width = 1320
    height = 470
    content: List[str] = ["0 0 0 rg", "0 0 0 RG"]
    content.extend(build_panel_pdf("Success Rate", "Rate", success, labels, 10, 20, 420, 410, 3, 0.0, 1.0, height))
    content.extend(
        build_panel_pdf(
            "Mean Reward (Success Only)",
            "Reward",
            reward,
            labels,
            450,
    ...
    content.extend(
        build_panel_pdf(
            ...
            410,
            1,
            math.floor(min(steps) / 20) * 20,
            math.ceil(max(steps) / 20) * 20,
            height,
        )
    )
...
def main() -> None:
    ...
    labels = [normalize_label(row["checkpoint"]) for row in rows]
    success = [float(row["success_rate"]) for row in rows]
    reward = [float(row["mean_reward_success"]) for row in rows]
    steps = [float(row["mean_steps_success"]) for row in rows]
    ...
    panel_h = 410
    panels = [
        build_panel(r"Success Rate", "Rate", success, labels, 10, 20, panel_w, panel_h, 3, 0.0, 1.0),
        build_panel(
            r"Mean Reward (Success Only)",
            "Reward",
            reward,
            labels,
            450,
...
"""
    ...
    write_pdf(rows, labels, success, reward, steps)
