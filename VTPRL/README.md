...
### 2.1 Design Rationale of Key Constants
The constants in this repository are not generic reward weights; they are tied directly to the warehouse HPRS specification in [`auto-shaping/configs/warehouse.yaml`](/Users/jerryplain/projects/HPRS_AGV/HPRS_AGV_Navigation/VTPRL/auto-shaping/configs/warehouse.yaml) and to the extra engineering terms implemented in [`auto-shaping/auto_shaping/hprs_vec_wrapper.py`](/Users/jerryplain/projects/HPRS_AGV/HPRS_AGV_Navigation/VTPRL/auto-shaping/auto_shaping/hprs_vec_wrapper.py).

`Safety enforcement.` In the current config, the only hard safety rule is `ensure "collision" <= 0.0`, which means collision avoidance has absolute priority in the hierarchy. The separate clearance term `achieve "min_laser" >= "laser_safe"` with `laser_safe = 1.5` is not a hard constraint; it is a soft safety margin that rewards the policy for keeping usable space around the robot before comfort terms are activated. In addition, `collision_penalty` is applied explicitly in the vectorized reward wrapper after HPRS shaping, so a collision causes both hierarchical gating failure and an extra negative reward. This design makes unsafe shortcutting unattractive even when progress shaping is strong.

`Goal-directed progress.` The main target quantity is not raw Euclidean distance alone, but the composite pose term `pose_error = dist_to_goal + 0.5 |delta_yaw|`. The threshold `approach_dist` therefore controls how early the agent is treated as being in a valid approach region while still coupling translation and heading. On top of the potential-based target shaping, the wrapper adds an extra dense term `delta_dist_weight * (d_t - d_{t+1})`. This is the repo's main progress bonus: it improves temporal credit assignment in long-horizon navigation and reduces hesitation near shelves or corners, while the HPRS part still preserves the intended safety-first ordering.

`Terminal approach refinement.` In this codebase, episode termination is ultimately decided by the environment success flag in [`agent/envs/warehouse_unity_env.py`](/Users/jerryplain/projects/HPRS_AGV/HPRS_AGV_Navigation/VTPRL/agent/envs/warehouse_unity_env.py), using `success_distance_threshold = 0.20` and `success_yaw_threshold = 2\pi` from [`agent/config.py`](/Users/jerryplain/projects/HPRS_AGV/HPRS_AGV_Navigation/VTPRL/agent/config.py). That means HPRS is not directly tightening termination itself. Instead, constants such as `approach_dist`, `yaw_comfort = 0.35`, `near_goal = 0.3`, and `very_near_goal = 0.2` refine behavior in the final approach region before the environment declares success. In practice, these terms reduce the chance that the policy hovers loosely near the target and encourage a cleaner, more deliberate final alignment.

`Motion-quality regulation.` The comfort layer uses `v_comfort = 0.6` and `omega_comfort = 0.4` to discourage overly aggressive linear and angular commands, and `pos_comfort = 2.0` to start rewarding controlled motion once the robot is already meaningfully close to the goal. Because comfort shaping is multiplied by both safety and target progress in the HPRS wrapper, smoothness is optimized only after the policy is already behaving safely and moving toward completion. This prevents comfort rewards from competing with core task completion during early or risky parts of an episode.
